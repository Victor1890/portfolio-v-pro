...
const useBookmarkBlogs = (key: string, defaultValue: []) => {
	...
	function removeFromBookmark(blogToRemove: string) {
		const data = getValue()
		setBookmarkedBlogs(data.filter((blog: FrontMatter) => blog.slug != blogToRemove))
	}

	function isAlreadyBookmarked(searchBySlug: string) {
		return bookmarkedBlogs?.map((bookmarkedBlog: IArticleDevTo) => bookmarkedBlog.slug === searchBySlug).includes(true)
	}
