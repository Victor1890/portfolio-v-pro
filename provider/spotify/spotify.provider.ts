...
import { GetTopArtistResponse, GetTopTrackResponse, ISpotifyAccessToken, ISpotifyCurrentlyPlaying } from "./spotify.interface";
...
class SpotifyProvider extends Base {
    ...
    public async getTopTracks(limit = 10): Promise<GetTopTrackResponse> {
        ...
        return this.get(`/v1/me/top/tracks?limit=${limit}&time_range=short_term`, {}, {
    ...
    public async getTopArtists(limit = 5): Promise<GetTopArtistResponse> {
        await this.getAccessToken()
        return this.get(`/v1/me/top/artists?limit=${limit}&time_range=short_term`, {}, {
            Authorization: `Bearer ${this.refresh_token}`,
        })
    }
