...
class DevToProvider extends Base {
  ...
  public async getPageOfPosts(
    page = 1,
    perPage = this.PER_PAGE_DEFAULT
  ): Promise<IArticleDevTo[]> {
    ...
    const data = await this.get<IArticleDevTo[]>(
      `/articles/me?per_page=${perPage}&page=${page}`
    );
  ...
  public async getArticleById(
    ...
    this.articleStorage.addOneArticle(
      articleId as string,
      data,
  ...
  public async getArticleBySlug(slug: string): Promise<IArticleDevTo> {
    ...
    const data = await this.get<IArticleDevTo>(
      `/articles/${devTo.username}/${slug}`
    );

    this.articleStorage.addOneArticle(
      slug,
      data,
      this.ONE_DAY_MILLISECONDS * 36000
    );
  ...
  public async getAllFollowers(
    ...
    const data = await this.get<IFollowDevTo[]>(
      `/followers/users?per_page=${perPage}&page=${page}`
    );
