...
import { FadeContainer, fromTopVariant, popUp } from '@content/FramerMotionVariants'
...
export default function Support() {
	...
	return (
		<section>
			...
			<AnimatedDiv variants={FadeContainer} className='grid gap-5 sm:grid-cols-3'>
				{support.map((paymentMethod) => {
					return (
						<Link target='_blank' rel='noopener noreferrer' key={paymentMethod.name} href={paymentMethod.url}>
							<motion.p
								variants={popUp}
...
function UPIPaymentForm({ close }: { close: () => void }) {
	...
	const generatePaymentQR = (e: FormEvent) => {
		e.preventDefault()
		setQrValue(`upi://pay?pa=${process.env.NEXT_PUBLIC_UPI}&pn=Jatin%20Sharma&am=${amount}&purpose=nothing&cu=INR`)
	}
	...
	return (
		...
		>
			...
			>
				...
				{!qrValue ? (
					<>
						<form onSubmit={generatePaymentQR} className='my-5 mb-10 flex flex-col gap-5'>
							<div className='relative flex items-center justify-center'>
								<BiRupee className='-ml-1 h-full w-9 text-gray-600 dark:text-gray-200' />
								<input
									onInput={(e: React.ChangeEvent<HTMLInputElement>) => {
										if (e.target.value.length === 0) return (e.target.style.width = '3ch')
					...
					<AnimatedDiv variants={FadeContainer} className='flex flex-col items-center'>
						<QRCode
