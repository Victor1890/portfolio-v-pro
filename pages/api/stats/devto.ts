...
export default async function handler(_req: NextRequest) {
  ...
  if (!followerData) {
    return NextResponse.json({ followers: 0, likes: 0, views: 0, comments: 0, posts: 0, },
      {
        headers: {
  ...
  return NextResponse.json(
    { followers, likes: totalLikes, views: totalViews, comments: totalComments, posts: totalPosts },
    {
      headers: {
        "Cache-Control": "public, s-maxage=86400, stale-while-revalidate=43200",
      },
