...
import { lockScroll, removeScrollLock } from '@utils/functions.util'
...
export default function TableOfContents({
	...
	useEffect(() => {
		setToc(
			tableOfContents.filter((table: any) => table.heading.toLowerCase().includes(searchValue.trim().toLowerCase()))
		)
			...
			{tableOfContents.length > 0 && (
				<>
					...
					>
							...
							{toc.map((content: any) => {
								return (
									<Link
										...
										className='relative overflow-hidden rounded-br-md rounded-tr-md border-l-2 border-neutral-500 px-2 py-0.5 font-medium text-neutral-700 hover:bg-darkSecondary  hover:text-white dark:text-neutral-200 dark:hover:border-white md:line-clamp-1 md:py-1'
						...
						{toc.length === 0 && <div className='text-center'>No Result found</div>}
					</div>
